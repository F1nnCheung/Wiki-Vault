...
- 如果你希望大模型输出在相同的问题下，输出的内容尽可能相同，你可以在每次模型调用时传入相同的seed值。
		- 如果你希望大模型在回答用户问题时不要总是用重复的句子，你可以适当调高 presence\_penalty 值。
		- 如果你希望查询事实性的内容，可以适当降低temperature 或 top\_p 值；反之，查询创造性的内容时，可以适当增加它们的值。
		- 如果你需要限制字数（如生成摘要、关键词）、控制成本或减少响应时间的场景，可以适当降低max\_tokens的值，但是若max\_tokens过小，可能会导致输出截断，反之，需要生成大段文本时，可以提高它的值。
...
- 重点：关注检索到的内容对不对；需要标注准确答案。
		- 输入：user\_input、reference（参考答案）、retrieved\_contexts
...
- 输入：user\_input，response，retrieved\_contexts。
...
- 计算造成敏感度的输入：user\_input、reference（参考答案）、response、retrieved\_contexts。
...
- 阿里云ACP课程：  
	https://github.com/AlibabaCloudDocs/aliyun\_acp\_learning/tree/main/%E5%A4%A7%E6%A8%A1%E5%9E%8BACP%E8%AE%A4%E8%AF%81%E6%95%99%E7%A8%8B
