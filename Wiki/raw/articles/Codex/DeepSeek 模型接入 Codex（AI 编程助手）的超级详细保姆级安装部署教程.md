鲲鹏论AI *2026年5月10日 22:04*

---

## deepseek-codex-install-guide summary: Windows + Mac 双平台完整教程，通过 mimo2codex 中间件 + CC Switch 实现 DeepSeek 模型接入 Codex，无需修改核心文件，安全稳定。 description: 本教程提供 Windows 和 Mac 双平台的 DeepSeek 模型接入 Codex 详细步骤，使用 mimo2codex 中间件本地转发方案，配合 CC Switch 配置管理工具，实现零修改核心文件的安全接入方式。
...
- • **DeepSeek API** ：提供模型能力
- ![图片](https://mmbiz.qpic.cn/mmbiz_png/hD3LeQ7JOicvNIElNPxrOxaDJULqXEiczw4L4g7rjndNwcpIM2HdBD9YPQhUpSEsgQkYaOEib3kRwnmnaoz5ukYYU5e2MgXojwicria0CWRcMNibw/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
按照 Codex 官方文档 安装 CLI 或桌面版，确保能正常启动。
...
1. 1\. 打开 **CC Switch** 应用
2. 2\. 切换到 **Codex** 标签页
3. 3\. 点击 **添加新供应商** → 选择 **自定义 / OpenAI 兼容**
4. 4\. 填写配置信息：
...
1. 5\. 在 config.toml 编辑区粘贴步骤 3 的内容，修改 `name` 为 `DEEPSEEK` ，保存并启用
2. 6\. CC Switch 会自动写入 Codex 配置文件
...
1. 1\. **重启 Codex** 应用
2. 2\. 在模型选择栏选择 **DEEPSEEK**
3. 3\. 发送测试指令，例如：
```
你好
```
1. 4\. 看到 DeepSeek 正常回复即表示接入成功
...
1. 1\. **安装依赖** ：Node.js、CC Switch、Codex
2. 2\. **部署中间件** ：全局安装并启动 mimo2codex
3. 3\. **配置连接** ：通过 CC Switch 添加 DeepSeek 供应商
4. 4\. **验证使用** ：重启 Codex 并测试模型响应
