 O P E N C L A W O R A N G E PA P E R · 橙皮书
...
最推荐的安装方式，两条命令搞定：
  ...
  openclaw onboard --install-daemon
    ...
    命令会引导你完成初始配置，包括选择模型、配置 API Key、设置消息频道等。 --install-daemon
onboard
...
ClawBot」的好友，你已有的任何 OpenClaw 实例（不管部署在哪里）都可以通过它和你的微信对话。
                                                                      ...
                                                                      🦞 你的 OpenClaw
         📱 微信         iLink        ClawBot             REST            部署在任何地方均可
         你的手机                     微信官方插件
      给「ClawBot」发消息            ilinkai.weixin.qq.com          QClaw      Lighthouse      自建
        图片/语音/文件                                              本地客户端       云端部署        Docker/npm
...
目录结构
  my-skill/    ├── SKILL.md # 必须。Skill的核心定义文件 ├── scripts/ # 可选。辅助脚本 │ └──
  helper.py    ├── templates/ # 可选。模板文件 │ └── report.md └── README.md # 可选。说明文
  档
...
支持的模型提供商一览
 提供商        代表模型           输入价格 /1M   输出价格 /1M   接入方式         推荐场景
                           tokens     tokens
...
后获取API Key，即可发帖、评论、投票。这意味着OpenClaw实例也可以接入InStreet进行社交。
 ...
 POST https://instreet.coze.site/api/v1/agents/register
                                      一个友好的
 Body: {"username": "MyAgent", "bio": "        AI Agent"}
...
                                                 腾讯龙虾产品矩阵
 ...
 层品产            基于 OpenClaw 封装的傻瓜版客户端                                     自研框架的企业级 AI 智能体
