...
sources:
  ...
  - raw/articles/Obsidian/Obsidian 入门45：对自己好一点，给 Obsidian 建个个人主页.md
related:
...
## 插件七：HomePage — 个人主页与启动面板

### 解决什么问题

Obsidian 默认打开上次关闭时的文件，但很多用户需要每次打开软件第一眼看到全景画面：最近在写什么、写了多少、今天有什么值得回顾。

**HomePage 插件**（社区插件市场搜索 "HomePage"）解决这个问题。核心功能只有两个：
1. 设置一个笔记作为启动页（"Open on startup"）
2. 关闭所有标签页时自动回到这个页面（"Open when empty"）

安装启用后，在设置中指定主页笔记路径即可，无复杂配置。

### 主页设计：三大经典模块

以林大友（2026.05）的实践为例，主页通常包含三个 Dataview 驱动的区块：

**① 足迹（活动热力图）**：配合 Activity Heatmap 或 Activity Graph 插件，类似 GitHub 绿格子，记录写作/笔记活动。哪一天写了多少字，颜色深浅一目了然。建议直接从社区装现成的热力图插件，省去自建开销。

**② 那年今日**：一个 Dataview 查询，找出所有创建日期是「今天」的笔记：
```dataview
TABLE WITHOUT ID
...
带来的体验是一种连续感——笔记库像一条流动的河，每一天都被过去某一天的自己提醒着。

**③ 今日随机推荐**：基于当天日期生成随机种子，每天随机推荐一篇旧笔记。同一天内刷新不变，保证"今天的惊喜"是固定的。基于日期种子的稳定性+意外性组合，让你经常翻到快忘记写过的笔记。

### 渐进式搭建建议

不用一步到位。最简单的版本：装 HomePage 插件→新建笔记→放几个常用链接，这就够了。等习惯有主页后再慢慢加 Dataview 查询、热力图等模块。核心问题是：**你每天打开 Obsidian 最想看什么？**
