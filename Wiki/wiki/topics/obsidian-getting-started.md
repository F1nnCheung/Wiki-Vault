...
sources:
  ...
  - raw/articles/Obsidian/我的Obsidian新主页，让我更期待每日打开记录.md
related:
...
当笔记越来越多，每次打开 Obsidian 不知道从哪看起——这是很多人遇到的瓶颈。解决方案是建一个「个人主页」作为每次打开的起点。

### 两种主页哲学

**功能中心型**（林大友方案）：用 HomePage 插件 + Dataview 查询构建仪表盘——活动热力图、那年今日、随机推荐。让你每次打开 Obsidian 第一眼看到全景画面。详见 [[obsidian-plugins-advanced|插件进阶指南]] 中 HomePage 插件的完整配置。

**生活空间型**（西湖太极熊方案）：反效率主义的主页设计。核心理念——"系统越来越轻，但人越来越稳定"：
- 无强压迫感、无大量数据库、无复杂 KPI
- 安静、克制、稳定、留白、节奏感
- 顶部是需求向往，中部是今日状态（安静工作/英语输入/散步放松），底部是完结状态
- 从"功能中心"变成情绪入口、节奏入口、身份入口、生活方式入口

这种设计特别适合长期创作者和深度工作型人格——降低注意力切换频率、认知疲劳和页面焦虑，从而提升稳定感和每日打开率。

> 💡 **核心问题**：无论哪种哲学，关键是先想清楚——**每天打开 Obsidian，我最想看到的那个画面是什么？**
...
5. **主页过度设计**：先放最核心的几个链接，等你习惯有主页后再慢慢加模块
